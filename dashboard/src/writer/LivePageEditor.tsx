...
// ─── Frontend content map ───
// Pre-populated with the real copy of every public page so the live editor
// always opens filled in. Content lives in src/config/frontendContent.ts.
import { FRONTEND_CONTENT } from '../config/frontendContent';
export { FRONTEND_CONTENT };
...
export default function LivePageEditor({
      ...
      if (data?.type === 'cms-section-click') {
        ...
        if (existing) {
          ...
          const newSection: Section = {
            ...
            type: data.kind ?? 'text',
            content: data.content || { heading: data.label ?? '', subtitle: '', buttonText: '' },
            style: { backgroundColor: '#ffffff', textColor: '#213448', alignment: 'left' },
            format: {},
          };
          setSections((prev) => [...prev, newSection]);
  ...
  const save = async () => {
    ...
    try {
      ...
      setToast('Unable to save changes. Please try again.');
