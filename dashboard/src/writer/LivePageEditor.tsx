...
function PreviewSection({ section, selected, onSelect }: any) {
  ...
  return (
    <div
      ...
      className={
        'LiveSection relative border-2 transition-all ' +
        (selected
          ? 'border-brand-yellow ring-2 ring-brand-yellow/40'
          : 'border-transparent hover:border-brand-yellow/60')
      }
      style={{
        background: bg,
        color,
        padding: st.padding || '2.5rem',
        textAlign: align,
        cursor: 'pointer',
      }}
    ...
    >
      ...
      <div className="max-w-5xl mx-auto space-y-3">
        ...
        {c.buttonText && (
          <span
            style={{
              ...
              marginTop: '0.75rem',
              background: st.accentColor || '#F4B400',
              color: '#213448',
        ...
        {Array.isArray(c.topLinks) && (
          ...
          >
            {c.topLinks.map((l: string, i: number) => (
...
export default function LivePageEditor({
  ...
  const load = async () => {
    ...
    try {
      ...
      if (list.length === 0 && initialSections && initialSections.length > 0) {
        ...
        try {
          await updateAllSections(slug, list);
        } catch {
          /* keep local copy */
        }
      }
  ...
  const addItem = (field: string) => {
    ...
    const items = [...cur];
    items.push(
      cur.length > 0 && typeof cur[0] === 'object'
        ? { name: 'New', text: 'Click to edit', rating: 5 }
        : 'New item'
    );
  ...
  const addSection = () => {
    ...
    const blank: any = {
      id,
      title: newType.charAt(0).toUpperCase() + newType.slice(1) + ' Section',
      type: newType,
      content: { heading: 'New Section Heading' },
      style: {
        backgroundColor: SECTION_BG[newType] || '#ffffff',
        textColor: '#213448',
        alignment: 'center',
      },
    };
    ...
    <div className="fixed inset-0 z-50 bg-gray-100 flex flex-col">
      <div className="bg-darkBlue text-white p-3 flex items-center justify-between shadow">
        <div className="flex items-center gap-3">
          <button
            onClick={onClose}
            className="text-sm border border-white/40 hover:bg-white/20 px-3 py-1.5 rounded-lg font-medium"
          >
            Back
          </button>
          <h2 className="font-bold text-lg">Live Editor - {title}</h2>
          {dirty && (
            <span className="text-xs bg-brand-yellow text-darkBlue px-2 py-0.5 rounded font-semibold">
              Unsaved changes
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={addSection}
            className="text-sm bg-brand-yellow text-darkBlue px-3 py-1.5 rounded-lg font-semibold"
          >
            + Add Section
          </button>
          <button
            ...
            className="text-sm bg-green-500 text-white px-4 py-1.5 rounded-lg font-semibold disabled:opacity-50"
          >
      ...
      {loading ? (
        <div className="flex items-center justify-center flex-1">
          <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-darkBlue"></div>
        </div>
      ) : (
        <div className="flex flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto bg-white flex flex-col">
            <div className="border-b border-gray-200 px-4 py-2 flex items-center justify-between bg-gray-50">
              <span className="text-xs font-semibold text-gray-500">
                {previewMode === 'real'
                  ? 'LIVE PREVIEW - real page - click any section to edit it'
                  : 'REPLICA PREVIEW - click any section to edit it'}
              </span>
              {path && (
          ...
          {selected ? (
            <div className="w-96 border-l border-gray-200 bg-white flex flex-col">
              <div className="p-4 border-b">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={removeCurrent} className="text-xs text-red-600 hover:underline">
                    Remove
                  </button>
                </div>
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <button
                    ...
                    className="text-xs border rounded px-1.5 hover:bg-gray-100"
                    title="Move up"
                  >
                    Up
                  </button>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div>
                  ...
                  {fields.length === 0 && (
                    <p className="text-xs text-gray-400">No rich fields for this section type.</p>
                  )}
                  {fields.map((f) => (
            ...
            <div className="w-96 border-l border-gray-200 bg-white flex items-center justify-center text-gray-400 px-6 text-center">
              Click a section in the live preview to edit its content and customization.
            </div>
          )}
        </div>
      )}
      ...
      {toast && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-darkBlue text-white px-5 py-2 rounded-lg shadow-lg text-sm">
          {toast}
        </div>
      )}
    </div>
  ...
  return (
    <div className="mt-2">
      <div className="flex items-center justify-between mb-1">
        ...
        <button onClick={onAdd} className="text-xs text-primary font-semibold hover:underline">
          + Add
        </button>
      </div>
      {items.map((it: any, i: number) => (
        <div key={i} className="flex items-center gap-2 mb-1">
          <input
            value={typeof it === 'object' ? it.name : it}
            onChange={(e) => onUpdate(i, e.target.value)}
            className="flex-1 border border-gray-300 rounded-lg px-2 py-1.5 text-sm"
          />
          <button onClick={() => onRemove(i)} className="text-xs text-red-600 hover:underline">
            x
          </button>
        </div>
      ))}
