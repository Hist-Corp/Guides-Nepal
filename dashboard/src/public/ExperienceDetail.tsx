...
export default function ExperienceDetail() {
  ...
  const exp = experiences.find((e) => (id && String(e.id) === id) || (slug && e.slug === slug));
  ...
  if (!exp) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-16 text-center">
        ...
        <Link
          to="/explore"
          className="inline-block mt-6 rounded-lg px-5 py-2 bg-darkBlue text-white font-semibold"
        >
  ...
  const related = experiences
    .filter((e) => e.id !== exp.id && (e.city === exp.city || e.category === exp.category))
    .slice(0, 3);
  ...
  async function handleBook() {
    ...
    try {
      ...
      setMessage(
        `Booked "${exp!.title}" for ${guests} guest(s) on ${date}. Check your dashboard for details.`
      );
    } catch (err: any) {
      ...
      <div className="bg-gradient-to-br from-lightBlue/60 to-peach/40">
        <div className="max-w-6xl mx-auto px-4 py-14">
          <img
            ...
            loading="lazy"
          />
          <div className="mt-3 text-xs uppercase tracking-wide text-gray-600">
            {exp.category} · {exp.city}
          </div>
          <h1 className="text-3xl md:text-4xl font-bold text-darkBlue mt-1">{exp.title}</h1>
          <div className="mt-2 text-sm text-gray-600">
            <span className="text-amber-500">? {exp.rating.toFixed(1)}</span> ({exp.reviews}{' '}
            reviews) � {exp.duration} � from{' '}
            <span className="font-bold text-darkBlue">${exp.price}</span> / person
          </div>
        </div>
      </div>
          ...
          <section>
            ...
            <ul className="space-y-2">
              {exp.highlights.map((h) => (
                <li key={h} className="flex gap-2 text-gray-600">
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-3 text-sm text-gray-600">
              <div className="rounded-xl bg-white border border-gray-200 p-4">
                ?? Meeting point: {exp.city} city center
              </div>
              <div className="rounded-xl bg-white border border-gray-200 p-4">
        ...
        <aside>
          <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-6 sticky top-24">
            <div className="text-xl font-semibold text-darkBlue">
              ${exp.price} <span className="text-sm font-normal text-gray-500">/ person</span>
            </div>
            ...
            {message && (
              <p className="mt-3 text-sm text-center text-darkBlue bg-lightBlue/50 rounded-lg px-3 py-2">
                {message}
              </p>
            )}
            <p className="text-xs text-gray-400 text-center mt-3">
              You won't be charged yet � the host confirms availability.
            </p>
          </div>
        </aside>
      </div>
