...
export function ExperienceCard({ exp }: { exp: PublicExperience }) {
  return (
    ...
    >
      <div className="h-36 bg-gradient-to-br from-lightBlue/60 to-peach/40">
        <img
          src={exp.image}
          alt={exp.title}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-2 text-sm">
          <span className="text-amber-500">
            ★ {exp.rating.toFixed(1)} <span className="text-gray-400">({exp.reviews})</span>
          </span>
          <span className="font-bold text-darkBlue">
            ${exp.price}{' '}
            <span className="text-xs text-gray-400 font-normal">/ person · {exp.duration}</span>
          </span>
        </div>
      </div>
    </Link>
...
export function ExperienceGrid({ items }: { items: PublicExperience[] }) {
  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((e) => (
        <ExperienceCard key={`${e.id}-${e.slug}`} exp={e} />
      ))}
    </div>
  );
}
